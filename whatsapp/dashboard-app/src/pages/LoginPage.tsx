...
import { AccountSelector, type LoginAccount } from '@/components/auth/AccountSelector'
...
const features = [
  { icon: Clock3, title: 'Gestion en temps réel', desc: 'Suivez votre activité en temps réel' },
  { icon: ShieldCheck, title: 'Sécurité avancée', desc: 'Vos données sont protégées' },
  { icon: Cloud, title: 'Accessible partout', desc: 'Accédez depuis n’importe où' },
]

function pickDefaultAccount(accounts: LoginAccount[]): number | null {
  if (!accounts.length) return null
  const admin = accounts.find((a) => a.role === 'admin')
  return admin?.id ?? accounts[0].id
}
...
export function LoginPage() {
  ...
  const loadAccounts = useCallback(async () => {
    ...
    setAccountsError('')
    try {
      const payload = await api<{ accounts: LoginAccount[] }>('/dashboard/api/auth/accounts')
      const list = payload.accounts || []
      setAccounts(list)
      setAccountId(pickDefaultAccount(list))
    } catch {
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      await login(accountId, values.password)
    } catch (err) {
          ...
          <div className="flex w-full max-w-[430px] flex-col overflow-hidden rounded-[26px] border border-border bg-white shadow-[0_20px_50px_rgba(16,42,67,0.1)]">
            <div className="px-7 py-6 sm:px-8 sm:py-7">
              <div className="mb-5 text-center">
                ...
                <p className="mt-1 text-xs text-muted">Accédez à votre espace</p>
              </div>
              ...
              <form className="space-y-3.5" onSubmit={onSubmit}>
                ...
                <div className="flex items-center justify-between gap-2">
                  <label className="inline-flex cursor-pointer items-center gap-2 text-sm text-text">
                    <input type="checkbox" className="h-3.5 w-3.5 accent-[#0F9FB2]" {...register('remember')} />
                    Se souvenir de moi
                  </label>
                  <button type="button" className="text-sm font-medium text-primary hover:underline" onClick={() => setForgotHint(true)}>
                    Mot de passe oublié ?
                  </button>
                </div>
                ...
                {error ? (
                  <p className="rounded-xl border border-danger/20 bg-danger/5 px-3 py-2 text-xs text-danger">{error}</p>
                ) : null}

                <Button
                  type="submit"
                  className="h-[42px] w-full rounded-[15px] text-sm"
            ...
            <div className="border-t border-border bg-[#F7FCFD] px-7 py-3.5 sm:px-8">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] leading-snug text-muted">Vos données sont chiffrées et protégées.</p>
                </div>
              </div>
            </div>
          </div>
