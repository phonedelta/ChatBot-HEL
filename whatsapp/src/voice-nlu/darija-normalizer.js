...
function classifyIntentFromConcepts(concepts, normalizedText = '') {
  ...
  if (c.has('my') && (c.has('appointment') || /\brdv\b|موعد/.test(text))) {
    return { intent: 'LIST_MY_APPOINTMENTS', confidence: 0.94, matched: 'concepts:my+appointment' }
  }
  if (/\b(rdv|rendez|موعد).{0,12}(dyali|diyali|ديالي)\b/.test(text)
  ...
  if (c.has('whether') && c.has('have') && c.has('appointment')) {
  ...
  // BOOK
  if (c.has('want') && c.has('appointment')) {
  ...
  if ((c.has('want') || /\bvouloir\b/.test(text)) && c.has('appointment')) {
  ...
  // FAQ
  if (c.has('where') || (c.has('where') && c.has('exist'))) {
    if (/\b(fin|fayn|فين)\b/.test(text)) {
      return { intent: 'ASK_LOCATION', confidence: 0.9, matched: 'concepts:where' }
    }
  }
  if (c.has('when') || (c.has('hours') && (c.has('what') || c.has('whether')))) {
